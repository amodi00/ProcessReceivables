...
sap.ui.define([
	...
	"use strict";
	var oMockServer,
		_sAppModulePath = "fin/ar/process/receivables/",
	...
	return {
		...
		init: function () {
			var oUriParameters = jQuery.sap.getUriParameters(),
				...
				oMainDataSource = oDataSource.situationsService,
			...
			aRequests.push({
                ...
                response: function (oXhr, sUrlParams) {
                    var oResponse = jQuery.sap.sjax({
                        url: sJsonFilesUrl + "/CheckAuth.json"
                    });
            ...
            aRequests.push({
                method: "POST",
                path: new RegExp("CheckAuthForCustomerContact(.*)"),
                response: function (oXhr, sUrlParams) {
                    var oResponse = jQuery.sap.sjax({
                        url: sJsonFilesUrl + "/CheckCreateCustomerContactAuth.json"
                    });
                    oXhr.respondJSON(200, {}, JSON.stringify(oResponse.data));
                    return true;
                }
            ...
            aRequests.push({
                method: "POST",
                path: new RegExp("ChangeStickyNoteContent(.*)"),
                response: function (oXhr, sUrlParams) {
                    var oResponse = jQuery.sap.sjax({
                        url: sJsonFilesUrl + "/ChangeStickyNoteResponse.json"
                    });
                    oXhr.respondJSON(200, {}, JSON.stringify(oResponse.data));
                    return true;
                }
